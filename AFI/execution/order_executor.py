...
class SmartOrderExecutor:
    ...
    def execute_trade(self, 
        ...
        """Execute full trade workflow with risk checks"""
        try:
            ...
            if isinstance(signal, str):
                ...
                signal = {
                    ...
                    'strategy_regime': signal.get('strategy_regime', 'default'),
                    'oco_enabled': signal.get('oco_enabled', False),
                    'stop_loss': signal.get('stop_loss'),
                    'take_profit': signal.get('take_profit')
                }
            else:
                logger.error(f"Invalid signal format for {token}: {signal}")
                return None

            # Validate required fields
            if not signal['direction']:
                logger.error(f"Missing direction in signal for {token}")
                return None

            # Validate market regime
            if not self._validate_regime(signal):
                logger.warning(f"Regime validation failed for {token}")
                return None

            # Validate token against current regime groups if market analyzer is available
            if self.market_analyzer:
                regime_groups, _ = self.market_analyzer.get_regime_groups()
                if not self._validate_regime_groups(token, regime_groups):
                    logger.warning(f"Token {token} not in valid regime group")
                    return None
    ...
    def _validate_regime(self, signal: Dict) -> bool:
        """Ensure trade aligns with current market regime"""
        current_regime = signal.get('regime', '')
        strategy_regime = signal.get('strategy_regime', '')
        return current_regime.lower() == strategy_regime.lower()

    def _validate_regime_groups(self, token: str, regime_groups: Dict) -> bool:
        """Validate token against current regime groups"""
        for group in regime_groups.values():
            if any(t['symbol'] == token for t in group):
                return True
        return False
